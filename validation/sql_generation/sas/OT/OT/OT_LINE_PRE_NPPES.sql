create temp table OT_LINE_PRE_NPPES distkey (ORGNL_CLM_NUM_LINE) sortkey (PRSCRBNG_PRVDR_NPI_NUM) as
select
    A.*,
    row_number() over (
...
from
    OT_LINE_IN as A
    inner join FA_HDR_OTHR_TOC H on H.TMSIS_RUN_ID = A.TMSIS_RUN_ID_LINE
