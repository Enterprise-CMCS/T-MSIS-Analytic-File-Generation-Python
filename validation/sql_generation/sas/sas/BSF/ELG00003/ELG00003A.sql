create temp table ELG00003A distkey(msis_ident_num) sortkey(submtg_state_cd, MSIS_IDENT_NUM) as
select
    a.TMSIS_RUN_ID,
    a.TMSIS_ACTV_IND,
    a.PRGNT_IND,
    ...
    a.TMSIS_RPTG_PRD
from
    cms_prod.TMSIS_VAR_DMGRPHC_ELGBLTY a
...
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            date_cmp(VAR_DMGRPHC_ELE_EFCTV_DT, '30NOV2021') in(-1, 0)
            and (
                date_cmp(VAR_DMGRPHC_ELE_END_DT, '01NOV2021') in(0, 1)
