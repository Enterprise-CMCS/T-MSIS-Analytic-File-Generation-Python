create
or replace temporary view ELG00014 as
select
    a.TMSIS_RUN_ID,
    a.TMSIS_ACTV_IND,
    case
    ...
    a.TMSIS_RPTG_PRD
from
    tmsis.TMSIS_MC_PRTCPTN_DATA as a -- left join data_anltcs_dm_prod.state_submsn_type s on a.submtg_state_cd = s.submtg_state_cd and upper(s.fil_type) = 'ELG'
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            MC_PLAN_ENRLMT_EFCTV_DT <= to_date('2021-11-30')
            and (
                MC_PLAN_ENRLMT_END_DT >= to_date('2021-11-01')
