create table Prov02_Main_Latest2 diststyle key distkey(submitting_state_prov_id) compound sortkey (
    tms_run_id,
    submitting_state,
...
where
    (
        date_cmp(T.prov_attributes_eff_date, '31DEC2017') in(-1, 0)
        and (
            date_cmp(T.prov_attributes_end_date, '01DEC2017') in(0, 1)
            or T.prov_attributes_end_date is NULL
        )
    )
    and (
        (
