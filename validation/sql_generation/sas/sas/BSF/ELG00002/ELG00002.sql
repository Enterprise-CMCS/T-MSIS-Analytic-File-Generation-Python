create temp table ELG00002 distkey(msis_ident_num) sortkey(submtg_state_cd, MSIS_IDENT_NUM) as
select
    a.TMSIS_RUN_ID,
    a.TMSIS_ACTV_IND,
    a.SUBMTG_STATE_CD,
    ...
    a.BIRTH_DT,
    case
        when upper(trim(a.GNDR_CD)) in('M', 'F') then upper(trim(a.GNDR_CD))
        else null
    end as GNDR_CD,
    a.ELGBL_1ST_NAME,
    a.ELGBL_LAST_NAME,
    ...
    a.TMSIS_RPTG_PRD
from
    cms_prod.TMSIS_PRMRY_DMGRPHC_ELGBLTY a
...
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            date_cmp(PRMRY_DMGRPHC_ELE_EFCTV_DT, '30NOV2021') in(-1, 0)
            and (
                date_cmp(PRMRY_DMGRPHC_ELE_END_DT, '01NOV2021') in(0, 1)
