...
select
    ...
    a.REC_NUM,
    a.RACE_CD,
    a.RACE_DCLRTN_EFCTV_DT,
    a.RACE_DCLRTN_END_DT,
    a.RACE_OTHR_TXT,
    case
        when upper(trim(a.CRTFD_AMRCN_INDN_ALSKN_NTV_IND)) in('0', '1', '2') then upper(trim(a.CRTFD_AMRCN_INDN_ALSKN_NTV_IND))
        else null
    end as CRTFD_AMRCN_INDN_ALSKN_NTV_IND,
    upper(a.MSIS_IDENT_NUM) as MSIS_IDENT_NUM,
    a.TMSIS_RPTG_PRD
from
    tmsis.TMSIS_RACE_INFO as a -- left join data_anltcs_dm_prod.state_submsn_type s on a.submtg_state_cd = s.submtg_state_cd and upper(s.fil_type) = 'ELG'
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            RACE_DCLRTN_EFCTV_DT <= to_date('2021-11-30')
            and (
                RACE_DCLRTN_END_DT >= to_date('2021-11-01')
                or RACE_DCLRTN_END_DT is NULL
            )
        )
    ) -- and (
    --     (
