create
or replace temporary view ELG00017 as
select
    a.TMSIS_RUN_ID,
    a.TMSIS_ACTV_IND,
    case
    ...
    a.REC_NUM,
    lpad(trim(a.DSBLTY_TYPE_CD), 2, '0') as DSBLTY_TYPE_CD,
    a.DSBLTY_TYPE_EFCTV_DT,
    a.DSBLTY_TYPE_END_DT,
    upper(a.MSIS_IDENT_NUM) as MSIS_IDENT_NUM,
    a.TMSIS_RPTG_PRD
from
    tmsis.TMSIS_DSBLTY_INFO as a -- left join data_anltcs_dm_prod.state_submsn_type s on a.submtg_state_cd = s.submtg_state_cd and upper(s.fil_type) = 'ELG'
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            DSBLTY_TYPE_EFCTV_DT <= to_date('2021-11-30')
            and (
                DSBLTY_TYPE_END_DT >= to_date('2021-11-01')
                or DSBLTY_TYPE_END_DT is NULL
            )
        )
    ) -- and (
    --     (
