create
or replace temporary view ELG00006 as
select
    a.TMSIS_RUN_ID,
    a.TMSIS_ACTV_IND,
    case
    ...
    a.HH_ENT_NAME,
    a.HH_SNTRN_NAME,
    a.HH_SNTRN_PRTCPTN_EFCTV_DT,
    a.HH_SNTRN_PRTCPTN_END_DT,
    upper(a.MSIS_IDENT_NUM) as MSIS_IDENT_NUM,
    a.TMSIS_RPTG_PRD
from
    tmsis.TMSIS_HH_SNTRN_PRTCPTN_INFO as a -- left join data_anltcs_dm_prod.state_submsn_type s on a.submtg_state_cd = s.submtg_state_cd and upper(s.fil_type) = 'ELG'
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            HH_SNTRN_PRTCPTN_EFCTV_DT <= to_date('2021-11-30')
            and (
                HH_SNTRN_PRTCPTN_END_DT >= to_date('2021-11-01')
                or HH_SNTRN_PRTCPTN_END_DT is NULL
            )
        )
    ) -- and (
    --     (
