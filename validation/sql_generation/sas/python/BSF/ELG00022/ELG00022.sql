create
or replace temporary view ELG00022 as
select
    a.TMSIS_RUN_ID,
    a.TMSIS_ACTV_IND,
    case
    ...
    a.ELGBL_ID_TYPE_CD,
    a.ELGBL_ID,
    a.ELGBL_ID_ISSG_ENT_ID_TXT,
    a.RSN_FOR_CHG,
    upper(a.MSIS_IDENT_NUM) as MSIS_IDENT_NUM,
    a.TMSIS_RPTG_PRD
from
    tmsis.TMSIS_ELGBL_ID as a -- left join data_anltcs_dm_prod.state_submsn_type s on a.submtg_state_cd = s.submtg_state_cd and upper(s.fil_type) = 'ELG'
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            ELGBL_ID_EFCTV_DT <= to_date('2021-11-30')
            and (
                ELGBL_ID_END_DT >= to_date('2021-11-01')
                or ELGBL_ID_END_DT is NULL
            )
        )
    ) -- and (
    --     (
