create
or replace temporary view IPL as
select
    6194 as DA_RUN_ID,
    cast (
        (
    ...
    case
        when (SRVC_BGNNG_DT < to_date('1600-01-01')) then to_date('1599-12-31')
        else nullif(SRVC_BGNNG_DT, '01JAN1960')
    end as SRVC_BGNNG_DT,
    case
        when (SRVC_ENDG_DT < to_date('1600-01-01')) then to_date('1599-12-31')
    ...
    case
        when IP_LT_ACTL_SRVC_QTY in (999999, 88888.888, 99999.99) then NULL
    ...
    case
        when ALOWD_AMT in (888888888.88, 99999999.0, 9999999999.99) then NULL
        else ALOWD_AMT
    end as ALOWD_AMT,
    case
    ...
    case
        when OTHR_INSRNC_AMT in (888888888.88, 88888888888.0, 88888888888.88) then NULL
        else OTHR_INSRNC_AMT
    end as OTHR_INSRNC_AMT,
    case
        when MDCD_FFS_EQUIV_AMT in (888888888.88, 88888888888.8, 999999.99) then NULL
