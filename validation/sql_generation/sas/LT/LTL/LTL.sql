CREATE TEMP TABLE LTL distkey(ORGNL_CLM_NUM) AS
SELECT
    6587 as DA_RUN_ID,
    cast (
        (
    ...
    case
        when BLG_UNIT_CD is NOT NULL
        and lpad(trim(BLG_UNIT_CD), 2, '0') in (
            '01',
            '02',
            '03',
            '04',
            ...
            '06',
            '07'
        ) then lpad(trim(BLG_UNIT_CD), 2, '0')
        else NULL
    end as BLG_UNIT_CD,
    case
        when (
    ...
    case
        when IP_LT_ACTL_SRVC_QTY in (88888.888, 99999.990, 999999) then NULL
        else IP_LT_ACTL_SRVC_QTY
    end as ACTL_SRVC_QTY,
    case
        when IP_LT_ALOWD_SRVC_QTY in (88888.888, 888888.890) then NULL
