create temp table ELG00022 distkey(msis_ident_num) sortkey(submtg_state_cd, MSIS_IDENT_NUM) as
select
    a.TMSIS_RUN_ID,
    a.TMSIS_ACTV_IND,
    a.SUBMTG_STATE_CD,
...
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            date_cmp(ELGBL_ID_EFCTV_DT, '30NOV2021') in(-1, 0)
            and (
                date_cmp(ELGBL_ID_END_DT, '01NOV2021') in(0, 1)
