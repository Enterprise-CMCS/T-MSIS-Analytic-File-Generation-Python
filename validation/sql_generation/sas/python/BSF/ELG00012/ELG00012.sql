create
or replace temporary view ELG00012 as
select
    a.TMSIS_RUN_ID,
    a.TMSIS_ACTV_IND,
    case
    ...
    a.REC_NUM,
    a.WVR_ENRLMT_EFCTV_DT,
    a.WVR_ENRLMT_END_DT,
    a.WVR_ID,
    case
        when upper(trim(a.WVR_TYPE_CD)) in(
            '1',
            '2',
            '3',
            '4',
            ...
            '32',
            '33'
        ) then upper(trim(a.WVR_TYPE_CD))
        else null
    end as WVR_TYPE_CD,
    upper(a.MSIS_IDENT_NUM) as MSIS_IDENT_NUM,
    a.TMSIS_RPTG_PRD
from
    tmsis.TMSIS_WVR_PRTCPTN_DATA as a -- left join data_anltcs_dm_prod.state_submsn_type s on a.submtg_state_cd = s.submtg_state_cd and upper(s.fil_type) = 'ELG'
where
    (
        a.TMSIS_ACTV_IND = 1
        and (
            WVR_ENRLMT_EFCTV_DT <= to_date('2021-11-30')
            and (
                WVR_ENRLMT_END_DT >= to_date('2021-11-01')
                or WVR_ENRLMT_END_DT is NULL
            )
        )
    ) -- and (
    --     (
