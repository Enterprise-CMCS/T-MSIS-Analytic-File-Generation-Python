CREATE TEMP TABLE IPH distkey(ORGNL_CLM_NUM) as
SELECT
    6000 as DA_RUN_ID,
    cast (
        (
            '0A' | | '-' | | 202110 | | '-' | | NEW_SUBMTG_STATE_CD | | '-' | | trim(COALESCE(NULLIF(ORGNL_CLM_NUM, '~'), '0')) | | '-' | | trim(COALESCE(NULLIF(ADJSTMT_CLM_NUM, '~'), '0')) | | '-' | | CAST(DATE_PART_YEAR(ADJDCTN_DT) AS CHAR(4)) | | CAST(DATE_PART(MONTH, ADJDCTN_DT) AS CHAR(2)) | | CAST(DATE_PART(DAY, ADJDCTN_DT) AS CHAR(2)) | | '-' | | COALESCE(ADJSTMT_IND_CLEAN, 'X')
        ) as varchar(126)
    ) as IP_LINK_KEY,
    '0A' as IP_VRSN,
    '202110' as IP_FIL_DT,
    ...
    case
        when date_cmp(ADMSN_DT, '1600-01-01') = -1 then '1599-12-31' :: date
        else ADMSN_DT
    end as ADMSN_DT,
    case
        ...
        and regexp_count(lpad(ADMSN_HR_NUM, 2, '0'), '[0-9]{2}') > 0 then case
            when (
                ADMSN_HR_NUM :: smallint >= 0
                and ADMSN_HR_NUM :: smallint <= 23
    ...
    case
        ...
        and regexp_count(lpad(DSCHRG_HR_NUM, 2, '0'), '[0-9]{2}') > 0 then case
            when (
                DSCHRG_HR_NUM :: smallint >= 0
                and DSCHRG_HR_NUM :: smallint <= 23
    ...
    case
        ...
        and lpad(trim(HOSP_TYPE_CD), 2, '0') in (
            '00',
            '01',
            '02',
            '03',
            ...
            '07',
            '08'
        ) then lpad(trim(HOSP_TYPE_CD), 2, '0')
        else NULL
    end as HOSP_TYPE_CD,
    case
    ...
    case
        when (
            upper(DGNS_POA_6_CD_IND) in ('Y', 'N', 'U', 'W', '1')
        ) then upper(DGNS_POA_6_CD_IND)
        else NULL
    ...
    case
        when (
            upper(DGNS_POA_7_CD_IND) in ('Y', 'N', 'U', 'W', '1')
        ) then upper(DGNS_POA_7_CD_IND)
        else NULL
    ...
    case
        when (
            upper(DGNS_POA_8_CD_IND) in ('Y', 'N', 'U', 'W', '1')
        ) then upper(DGNS_POA_8_CD_IND)
        else NULL
    ...
    case
        when (
            upper(DGNS_POA_9_CD_IND) in ('Y', 'N', 'U', 'W', '1')
        ) then upper(DGNS_POA_9_CD_IND)
        else NULL
    ...
    case
        when (
            upper(DGNS_POA_10_CD_IND) in ('Y', 'N', 'U', 'W', '1')
        ) then upper(DGNS_POA_10_CD_IND)
        else NULL
    ...
    case
        when (
            upper(DGNS_POA_11_CD_IND) in ('Y', 'N', 'U', 'W', '1')
        ) then upper(DGNS_POA_11_CD_IND)
        else NULL
    ...
    case
        when (
            upper(DGNS_POA_12_CD_IND) in ('Y', 'N', 'U', 'W', '1')
        ) then upper(DGNS_POA_12_CD_IND)
        else NULL
    ...
    case
        when date_cmp(PRCDR_1_CD_DT, '1600-01-01') = -1 then '1599-12-31' :: date
        else PRCDR_1_CD_DT
    ...
    case
        when date_cmp(PRCDR_2_CD_DT, '1600-01-01') = -1 then '1599-12-31' :: date
        else PRCDR_2_CD_DT
    ...
    case
        when date_cmp(PRCDR_3_CD_DT, '1600-01-01') = -1 then '1599-12-31' :: date
        else PRCDR_3_CD_DT
    ...
    case
        when date_cmp(PRCDR_4_CD_DT, '1600-01-01') = -1 then '1599-12-31' :: date
        else PRCDR_4_CD_DT
    ...
    case
        when date_cmp(PRCDR_5_CD_DT, '1600-01-01') = -1 then '1599-12-31' :: date
        else PRCDR_5_CD_DT
    ...
    case
        when date_cmp(PRCDR_6_CD_DT, '1600-01-01') = -1 then '1599-12-31' :: date
        else PRCDR_6_CD_DT
    ...
    case
        when lpad(OUTLIER_CD, 2, '0') in ('03', '04', '05') then NULL
            ...
            and regexp_count(lpad(outlier_cd, 2, '0'), '[0-9]{2}') > 0 then case
                when (
                    outlier_cd :: smallint >= 0
                    and outlier_cd :: smallint <= 10
    ...
    case
        when regexp_count(lpad(ADMTG_PRVDR_SPCLTY_CD, 2, '0'), '[0-9]{2}') > 0 then case
            when (
                ADMTG_PRVDR_SPCLTY_CD :: smallint >= 1
                and ADMTG_PRVDR_SPCLTY_CD :: smallint <= 98
    ...
    case
        when regexp_count(lpad(admtg_prvdr_type_cd, 2, '0'), '[0-9]{2}') > 0 then case
            when (
                admtg_prvdr_type_cd :: smallint >= 1
                and admtg_prvdr_type_cd :: smallint <= 57
