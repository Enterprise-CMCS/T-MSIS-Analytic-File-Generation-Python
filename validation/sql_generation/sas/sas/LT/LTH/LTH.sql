CREATE TEMP TABLE LTH distkey(ORGNL_CLM_NUM) AS
SELECT
    6587 as DA_RUN_ID,
    cast (
        (
            '46' || '-' || 201712 || '-' || NEW_SUBMTG_STATE_CD || '-' || trim(COALESCE(NULLIF(ORGNL_CLM_NUM, '~'), '0')) || '-' || trim(COALESCE(NULLIF(ADJSTMT_CLM_NUM, '~'), '0')) || '-' || CAST(DATE_PART_YEAR(ADJDCTN_DT) AS CHAR(4)) || CAST(DATE_PART(MONTH, ADJDCTN_DT) AS CHAR(2)) || CAST(DATE_PART(DAY, ADJDCTN_DT) AS CHAR(2)) || '-' || COALESCE(ADJSTMT_IND_CLEAN, 'X')
        ) as varchar(126)
    ) as LT_LINK_KEY,
    '46' as LT_VRSN,
    '201712' as LT_FIL_DT,
    ...
    case
        when date_cmp(DSCHRG_DT, '1600-01-01') = -1 then '1599-12-31' :: date
        else DSCHRG_DT
    end as DSCHRG_DT,
    case
