create
or replace temporary view OTL as
select
    6194 as DA_RUN_ID,
    'ot_link_key' as OT_LINK_KEY,
    '0A' as OT_VRSN,
    '202110' as OT_FIL_DT,
    ...
    case
        ...
        else nullif(ADJDCTN_DT_LINE, to_date('1960-01-01'))
    end as ADJDCTN_DT,
    ...
    case
        when date_cmp(SRVC_BGNNG_DT_LINE, to_date('1600-01-01')) then to_date('1599-12-31')
        else nullif(SRVC_BGNNG_DT_LINE, to_date('1960-01-01'))
    end as SRVC_BGNNG_DT,
    case
        when date_cmp(SRVC_ENDG_DT_LINE, to_date('1600-01-01')) then to_date('1599-12-31')
        else nullif(SRVC_ENDG_DT_LINE, to_date('1960-01-01'))
    end as SRVC_ENDG_DT,
    case
    ...
    case
        when (PRCDR_CD_DT < to_date('1600-01-01')) then to_date('1599-12-31')
        else PRCDR_CD_DT
    end as PRCDR_CD_DT,
    case
    ...
    case
        when OTHR_TOC_RX_CLM_ALOWD_QTY in (999999.000, 888888.000, 888888.880, 99999.999, 99999) then NULL
    ...
    case
        when lpad(TOOTH_ORAL_CVTY_AREA_DSGNTD_CD, 2, '0') in ('20', '30', '40') then lpad(TOOTH_ORAL_CVTY_AREA_DSGNTD_CD, 2, '0')
            ...
            and regexp_count(
                ...
                when (
                    TOOTH_ORAL_CVTY_AREA_DSGNTD_CD >= 0
                    and TOOTH_ORAL_CVTY_AREA_DSGNTD_CD <= 10
