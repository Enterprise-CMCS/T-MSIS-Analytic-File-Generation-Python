CREATE TEMP TABLE RXL distkey(ORGNL_CLM_NUM) AS
select
    6635 as DA_RUN_ID,
    cast (
        (
            '46' || '-' || 201712 || '-' || NEW_SUBMTG_STATE_CD_LINE || '-' || trim(COALESCE(NULLIF(ORGNL_CLM_NUM_LINE, '~'), '0')) || '-' || trim(COALESCE(NULLIF(ADJSTMT_CLM_NUM_LINE, '~'), '0')) || '-' || CAST(DATE_PART_YEAR(ADJDCTN_DT_LINE) AS CHAR(4)) || CAST(DATE_PART(MONTH, ADJDCTN_DT_LINE) AS CHAR(2)) || CAST(DATE_PART(DAY, ADJDCTN_DT_LINE) AS CHAR(2)) || '-' || COALESCE(LINE_ADJSTMT_IND_CLEAN, 'X')
        ) as varchar(126)
    ) as RX_LINK_KEY,
    '46' as RX_VRSN,
    '201712' as RX_FIL_DT,
    ...
    case
        when trim(upper(UOM_CD)) in ('F2', 'ML', 'GR', 'UN', 'ME', 'EA', 'GM') then trim(upper(UOM_CD))
        else NULL
    end as UOM_CD,
    case
    ...
    case
        ...
        and regexp_count(lpad(NEW_REFL_IND, 2, '0'), '[0-9]{2}') > 0 then case
            when (
                NEW_REFL_IND :: smallint >= 0
                and NEW_REFL_IND :: smallint <= 99
    ...
    case
        ...
        and trim(BRND_GNRC_IND) in ('0', '1', '2', '3', '4') then trim(BRND_GNRC_IND)
        else NULL
    ...
    case
        when lpad(CMPND_DSG_FORM_CD, 2, '0') in ('08', '09') then NULL
            ...
            and regexp_count(lpad(CMPND_DSG_FORM_CD, 2, '0'), '[0-9]{2}') > 0 then case
                when (
                    CMPND_DSG_FORM_CD :: smallint >= 1
                    and CMPND_DSG_FORM_CD :: smallint <= 18
    ...
    case
        when othr_toc_rx_clm_alowd_qty in (99999, 99999.999, 888888.000, 999999, 888888.880) then NULL
        else othr_toc_rx_clm_alowd_qty
    end as alowd_srvc_qty,
    case
        when othr_toc_rx_clm_actl_qty in (999999.99, 888888, 999999, 0) then NULL
    ...
    case
        when bill_amt in (9999999999.99, 999999.99, 999999, 888888888.88) then NULL
        else bill_amt
    end as bill_amt,
    case
        when alowd_amt in (9999999999.99, 888888888.88, 99999999.00) then NULL
        else alowd_amt
    end as alowd_amt,
    case
    ...
    case
        when mdcd_ffs_equiv_amt in (999999.99, 888888888.88, 88888888888.80) then NULL
    ...
    case
        when othr_insrnc_amt in (88888888888.00, 88888888888.88, 888888888.88) then NULL
